document.addEventListener("DOMContentLoaded", () => {
    ...
    const requiredFieldNames = ["customerCode", "customerName", "gatePassDate"];
    ...
    const requiredFields = ['gatePassNo', 'customerCode', 'customerName', 'gatePassDate', 'gatePassType', 
                               'noOfPacks', 'description', 'quantity', 'purpose'];
    ...
    if (missingFields.length > 0) {
        ...
        const url = isEditing ? `/update_gate_passes/${editingGatePassId}` : '/add_update_gate_pass';
    ...
    const renderPage = (data, page) => {
        ...
        pageData.forEach(gatePass => {
            ...
            // Define the specific order of fields to display in table
            const fieldOrder = [
...
deleteCell.addEventListener("click", (event) => {
    ...
    if (confirm("Are you sure you want to delete this Update Gate Pass?")) {
        fetch(`/delete_gate_passes/${requestId}`, {
            method: 'DELETE'
        })
            .then(response => response.json())
            .then(data => {
